...
{
    while (!dkpf_cbor_value_at_end(it)) {
        CborError err;
        ...
        case CborMapType: {
            // recursive type
            CborValue recursed;
            assert(dkpf_cbor_value_is_container(it));
        ...
        case CborIntegerType: {
            int64_t val;
            dkpf_cbor_value_get_int64(it, &val);     // can't fail
            printf("%lld\n", (long long)val);
            break;
        }
        ...
        case CborByteStringType: {
            uint8_t *buf;
            size_t n;
            err = dkpf_cbor_value_dup_byte_string(it, &buf, &n, it);
            if (err)
                return err;     // parse error
            dumpbytes(buf, n);
            puts("");
        ...
        case CborTextStringType: {
            char *buf;
            size_t n;
            err = dkpf_cbor_value_dup_text_string(it, &buf, &n, it);
            if (err)
                return err;     // parse error
            puts(buf);
            free(buf);
        ...
        case CborSimpleType: {
            uint8_t type;
            dkpf_cbor_value_get_simple_type(it, &type);  // can't fail
            printf("simple(%u)\n", type);
            break;
        }
        ...
        case CborHalfFloatType: {
            uint16_t val;
            dkpf_cbor_value_get_half_float(it, &val);
            printf("__f16(%04x)\n", val);
            break;
        }
...
int main(int argc, char **argv)
{
    ...
    CborParser parser;
    CborValue it;
    CborError err = dkpf_cbor_parser_init(buf, length, 0, &parser, &it);
    if (!err)
    ...
    if (err) {
        fprintf(stderr, "CBOR parsing failure at offset %ld: %s\n",
                it.ptr - buf, dkpf_cbor_error_string(err));
        return 1;
    }
    return 0;
}
